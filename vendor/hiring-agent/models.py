...
class OllamaProvider:
    ...
    def __init__(self):
        ...
        # ponytail: support Ollama Cloud (or any custom host) via env vars.
        # Falls back to the default local module client when neither is set.
        host = os.getenv("OLLAMA_HOST")
        api_key = os.getenv("OLLAMA_API_KEY")
        if host or api_key:
            headers = {"Authorization": f"Bearer {api_key}"} if api_key else None
            self.client = ollama.Client(
                host=host or "http://localhost:11434", headers=headers
            )
        else:
            self.client = ollama
